...
#   5) Delete checkpoint/model weight files after runs finish.
#
# Outputs:
...
# Always avoid keeping model weights/checkpoints.
# The base launcher already supports DELETE_CKPT_AFTER_TEST; this wrapper
# forces it on and also performs a final cleanup pass under OUTPUT_ROOT.
...
trap cleanup_weight_files EXIT
...
echo "[config] CLEAN_WEIGHTS_AFTER_RUN=${CLEAN_WEIGHTS_AFTER_RUN}"
...
cleanup_weight_files
trap - EXIT
echo "[done] focused unified BayesRT sweep + unified-tag confirm/B2N finished; checkpoint/model weights deleted."
