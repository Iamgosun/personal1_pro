...
class CLIPFeatureExtractor:
    ...
    def _build_loader(self, split: str, train_aug: bool):
        ...
        dataset = build_split_dataset(self.trainer.cfg, data_source, is_train=(train_aug if split == 'train' else default_train))
        sampler = self.trainer.train_loader_x.sampler if split == 'train' else SequentialSampler(dataset)
        batch_size = self.trainer.cfg.DATALOADER.TRAIN_X.BATCH_SIZE if split == 'train' else self.trainer.cfg.DATALOADER.TEST.BATCH_SIZE
        return DataLoader(dataset, batch_size=batch_size, sampler=sampler, num_workers=0, pin_memory=torch.cuda.is_available(), drop_last=False)

    def extract_split(self, split: str, reps: int = 1, train_aug: bool = False, mode: str = 'features_only') -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if cached is not None:
            return cached['labels'].to(self.trainer.device), cached['logits'].to(self.trainer.device), cached['features'].to(self.trainer.device)
        ...
        self.cache_manager.save(spec, {'labels': labels, 'logits': logits, 'features': features, 'meta': {'num_samples': int(labels.shape[0]), 'feature_dim': int(features.shape[-1])}})
