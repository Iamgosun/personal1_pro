...
class GaussianPerClassAdapter(BaseAdapter):
    ...
    def __init__(self, cfg, clip_model, base_text_features: torch.Tensor):
        ...
        self.variational_log_sigma = nn.Parameter(torch.full((n_classes, feat_dim), math.log(0.01), device=base_text_features.device))
        self.register_buffer("prior_mu", base_text_features.clone())
        self.register_buffer("prior_sigma", torch.full((n_classes, feat_dim), 0.01, device=base_text_features.device))
    ...
    def forward(self, n_samples: int = 1) -> torch.Tensor:
        epsilon = torch.randn((n_samples, *self.variational_log_sigma.shape), device=self.variational_mu.device)
