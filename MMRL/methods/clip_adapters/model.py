...
class ClipAdaptersModel(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        if cfg.CLIP_ADAPTERS.ENHANCED_BASE == 'none':
            base_text_features, text_embeddings_all = build_base_text_features(cfg, classnames, clip_model, text_encoder)
        else:
            base_text_features, text_embeddings_all = build_base_text_features(cfg, classnames, clip_model, text_encoder, cfg.CLIP_ADAPTERS.ENHANCED_BASE)
    ...
    def forward_features(self, features):
        ...
        if 'CrossModal' in init or init == 'RANDOM' or 'ZS' in init:
            prototypes = self.adapter()
            return lp_logits(self.logit_scale, features, prototypes)
        prototypes = self.adapter(n_samples=self.adapter.cfg.CLIP_ADAPTERS.N_SAMPLES if hasattr(self.adapter, 'cfg') else 3)
...
class ClipAdaptersMethod(BaseMethod):
    ...
    def forward_train(self, batch):
        ...
        if 'features' in batch:
            ...
            return MethodOutputs(logits=logits, labels=label, features={'img': features}, extras={'mode': 'cache'})
        image = batch['img'].to(self.device)
        logits, image_features = self.model(image, return_features=True)
        return MethodOutputs(logits=logits, labels=label, features={'img': image_features}, extras={'mode': 'online'})
