...
#      MAIN_CONSISTENCY_MODE on 16-shot seed1.
  ...
  for arg in "$@"; do
    if [[ "${arg}" == *=* ]]; then
      ...
      case "${key}" in
        ...
        BAYES_REP_PRIOR_STD_LIST|BAYES_REP_KL_WEIGHT_LIST|BAYES_MAIN_CONSISTENCY_MODE_LIST|\
        SUMMARY_ONLY)
          printf -v "${key}" '%s' "${val}"
          export "${key}"
          ;;
...
VCRM_ETA_LIST=${VCRM_ETA_LIST:-"0.02  0.2 0.5"}
...
BAYES_MAIN_CONSISTENCY_MODE_LIST=${BAYES_MAIN_CONSISTENCY_MODE_LIST:-"prob "}
    ...
    for kl in ${BAYES_REP_KL_WEIGHT_LIST}; do
      for mode in ${BAYES_MAIN_CONSISTENCY_MODE_LIST}; do
        register_case BAYES_TAGS BAYES_OPTS \
          "bayes_pstd-${prior_std}_kl-${kl}_cons-${mode}" \
...
run_case_b2n() {
  ...
  if [[ "${SKIP_EXISTING}" == "1" && -f "${eval_outdir}/test_metrics.json" ]]; then
    ...
    cleanup_checkpoint_if_ready "${train_outdir}" "${train_log}"
    cleanup_checkpoint_if_ready "${eval_outdir}" "${eval_log}"
    return 0
  fi
